...
// Define una customización de la funcion flexible cubica (Tarjeta)
CustomEase.create("cubic", "0.83, 0, 0.17, 1");

// Variable para rastrear si una animación está actualmente en progreso.
let isAnimating = false;

// Función para dividir el texto de los elementos en espacios para cada carácter
...
function initializeCards() {
  let cards = Array.from(document.querySelectorAll(".card.targets"));
  gsap.to(cards, {
    y: (i) => -15 + 15 * i + "%",
    z: (i) => 15 * i,
    duration: 1,
...
document.addEventListener("DOMContentLoaded", () => {
  // Divide el texto en elementos de h1 con la clase "copy"
  splitTextIntoSpans(".copy.tarjeta h1.textPlan");
  // Inicializa las tarjetas
  initializeCards();

  // Inicia los elementos al ver el h1
  gsap.set("h1.textPLan span", { y: -200 });
  gsap.set(".slider.deslizar .card.targets:last-child h1.textPlan span", {
    y: 0,
  });
});

// Escucha el evento al clickear
document.addEventListener("click", () => {
  // Si una animacion esta lista en el proceso, retornara rapido
  if (isAnimating) return;

  // Establece la animacion en verdadero para mostrar el inicio de la animación
  isAnimating = true;
  // Selecciona los sliders en la tarjeta
  let slider = document.querySelector(".slider.deslizar");
  let cards = Array.from(slider.querySelectorAll(".card.targets"));
  let lastCard = cards.pop(); // Elimina la tarjeta anterior
  let nextCard = cards[cards.length - 1]; // Obtiene una nueva tarjeta

  // Anima la ultima tarjeta para moverla abajo
  gsap.to(lastCard.querySelectorAll("h1.textPlan span"), {
    y: 200,
    duration: 0.75,
    ease: "cubic",
  });
  ...
  gsap.to(lastCard, {
    ...
    ease: "cubic",
    onComplete: () => {
      // Cuando las animaciones terminen, pondra la animación al frente
      slider.prepend(lastCard);
      // Reinicia las tarjetas
      initializeCards();
      // Resetea la posición de los elementos
      gsap.set(lastCard.querySelectorAll("h1.textPlan span"), { y: -200 });

      // Establece la animación en falso
      setTimeout(() => {
        isAnimating = false;
      }, 1000);
    },
  ...
  // Animacion para la siguiente carta
  gsap.to(nextCard.querySelectorAll("h1.textPlan span"), {
    y: 0,
    duration: 1,
    ease: "cubic",
    stagger: 0.05,
...
$("#iniciar_chat").click(function () {
  ...
  if ($("#nombre_whatsapp").val() == "" || $("#nombre_whatsapp").val() == " ") {
    ...
    $("#telefono_whatsapp").val() == " " ||
    $("#telefono_whatsapp").hasClass("invalid")
  ) {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (
          url_archivo[url_archivo.length - 1] == "" ||
          url_archivo[url_archivo.length - 1] == "index.php"
        ) {
          //Para index
          url = "php/enviar.php";
        }
        $.ajax({
          ...
          success: function (result) {
            window.open(
              "https://api.whatsapp.com/send?phone=573015350964&text=" +
                "Hola, mi nombre es: " +
                $("#nombre_whatsapp").val() +
                ". Les escribo porque: " +
                $("#mensaje_whatsapp").val()
...
$("#enviar_mensaje").click(function () {
  if (mensajes_enviados == 6) {
    ...
    if (
      mensajes_enviados != 2 &&
      mensajes_enviados != 3 &&
      ($("#mensaje_chatbot").val() == "" || $("#mensaje_chatbot").val() == " ")
    ) {
      $("#mensaje_chatbot").focus();
      M.toast({
        html: "Necesita colocar un mensaje",
      ...
      ($("#mensaje_chatbot").val() == "" ||
        $("#mensaje_chatbot").val() == " " ||
        $("#mensaje_chatbot").hasClass("invalid"))
    ) {
      $("#mensaje_chatbot").focus();
      M.toast({
        html: "Necesita colocar un teléfono valido",
      ...
      (mensajes_enviados == 3 && $("#mensaje_chatbot").hasClass("invalid")) ||
      (mensajes_enviados == 3 && correo_test.length <= 1)
    ) {
      $("#mensaje_chatbot").focus();
      M.toast({
      ...
      $("#mensaje_chatbot").val("");
      $(".contenido-chat-chatbot").scrollTop(
        document.getElementById("contenido-chat-chatbot").scrollHeight
      );
      setTimeout(function () {
        $("#enviar_mensaje").find("i").css({
          left: "0px",
    ...
    setTimeout(function () {
      ...
      $(".contenido-chat-chatbot").append(
        '<div class="col s12 no-padding nombre-bot">' +
          ...
          '<div class="col s10 mensaje-bot white-text text-darken-4">' +
          nombre_usuario +
          " yo me encargo de ponerte en contacto con un asesor para que responda tu consulta" +
          "</div>" +
          '<div class="col s10 mensaje-bot white-text text-darken-4">' +
    ...
    setTimeout(function () {
      ...
      $(".contenido-chat-chatbot").append(
        '<div class="col s12 no-padding nombre-bot">' +
          ...
          '<div class="col s10 mensaje-bot white-text text-darken-4">' +
          nombre_usuario +
          " con la información que me has pasado un asesor se comunicará contigo a la brevedad." +
          "</div>"
      );
      $(".contenido-chat-chatbot").scrollTop(
        document.getElementById("contenido-chat-chatbot").scrollHeight
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  filterForm.addEventListener("change", () => {
    const checkedCategories = Array.from(
      filterForm.querySelectorAll("input:checked")
    ).map((input) => input.name);

    articles.forEach((article) => {
      const articleCategory = article.getAttribute("data-category");
      if (checkedCategories.includes(articleCategory)) {
  ...
  // Trigger the change event on page load to set initial filter state
